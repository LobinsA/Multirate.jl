...
function FIRRational( h::Vector, ratio::Rational )
    ...
    N𝜙           = size( pfb )[2]
    tapsPer𝜙     = size( pfb )[1]
    criticalYidx = floor( Int, tapsPer𝜙 * ratio )
    𝜙Idx         = 1
    inputDeficit = 1
...
function filt!{Tb,Th,Tx}( buffer::Vector{Tb}, self::FIRFilter{FIRRational{Th}}, x::Vector{Tx} )
    ...
    while inputIdx <= xLen
        ...
        inputIdx      += floor( Int, ( kernel.𝜙Idx + decimation - 1 ) / interpolation )
...
function update( kernel::FIRArbitrary )
    ...
    if kernel.𝜙Accumulator > kernel.N𝜙
        kernel.xIdx        += floor( Int, (kernel.𝜙Accumulator-1) / kernel.N𝜙 )
...
function update( kernel::FIRFarrow )
    ...
    if kernel.𝜙Idx > kernel.N𝜙
        kernel.xIdx += floor( Int, (kernel.𝜙Idx-1) / kernel.N𝜙 )
...
#==============================================================================#
#              ____ _  _ ____ _  _ _  _ ____ _    _ ___  ____ ____             #
#              |    |__| |__| |\ | |\ | |___ |    |   /  |___ |__/             #
#              |___ |  | |  | | \| | \| |___ |___ |  /__ |___ |  \             #
#==============================================================================#
include( "Channelizer.jl" )
